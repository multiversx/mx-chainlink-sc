...
pub trait Aggregator {
    ...
    fn description(&self) -> SingleValueMapper<BoxedBytes>;
    ...
    fn get_deposit(&self, address: &ManagedAddress) -> BigUint {
        self.deposits().get(address).unwrap_or_else(|| BigUint::zero())
    }
    ...
    fn validate_submission_limits(&self, submission_values: &Vec<BigUint>) -> SCResult<()> {
    ...
    fn submit(
        &self,
        round_id: u64,
        #[var_args] submission_values: VarArgs<BigUint>,
    ) -> SCResult<()> {
        require!(
    ...
    fn change_oracles(
        &self,
        removed: Vec<ManagedAddress>,
        added: Vec<ManagedAddress>,
        added_admins: Vec<ManagedAddress>,
        min_submissions: u64,
        max_submissions: u64,
        restart_delay: u64,
    ) -> SCResult<()> {
    ...
    #[view(getRoundData)]
    fn get_round_data(&self, round_id: u64) -> OptionalResult<Round<Self::Api>> {
        self.rounds().get(&round_id).into()
    }

    #[view(latestRoundData)]
    fn latest_round_data(&self) -> OptionalResult<Round<Self::Api>> {
        self.get_round_data(self.latest_round_id().get())
    }

    #[view(withdrawablePayment)]
    fn withdrawable_payment(&self, oracle: ManagedAddress) -> SCResult<BigUint> {
    ...
    #[view(getAdmin)]
    fn get_admin(&self, oracle: ManagedAddress) -> SCResult<ManagedAddress> {
        Ok(self.get_oracle_status_result(&oracle)?.admin)
    }

    #[endpoint(transferAdmin)]
    fn transfer_admin(&self, oracle: ManagedAddress, new_admin: ManagedAddress) -> SCResult<()> {
        let mut oracle_status = self.get_oracle_status_result(&oracle)?;
        require!(
            oracle_status.admin == self.blockchain().get_caller(),
            "only callable by admin"
    ...
    fn accept_admin(&self, oracle: ManagedAddress) -> SCResult<()> {
        let mut oracle_status = self.get_oracle_status_result(&oracle)?;
        let caller = self.blockchain().get_caller();
        require!(
    ...
    ) -> SCResult<OracleRoundState<Self::Api>> {
    ...
    fn subtract_amount_from_deposits(&self, amount: &BigUint) {
        ...
        let mut final_amounts: Vec<(ManagedAddress, BigUint)> = Vec::new();
    ...
    fn record_submission(
        &self,
        submission: Submission<Self::Api>,
        round_id: u64,
    ) -> SCResult<()> {
        require!(
    ...
    fn add_oracle(&self, oracle: &ManagedAddress, admin: &ManagedAddress) -> SCResult<()> {
    ...
    fn delayed(&self, oracle: &ManagedAddress, round_id: &u64) -> SCResult<bool> {
        let oracle_status = self.get_oracle_status_result(oracle)?;
    ...
    fn get_oracle_status_result(&self, oracle: &ManagedAddress) -> SCResult<OracleStatus<Self::Api>> {
    ...
    fn get_round(&self, round_id: &u64) -> SCResult<Round<Self::Api>> {
    ...
    fn get_round_details(&self, round_id: &u64) -> SCResult<RoundDetails<Self::Api>> {
    ...
    fn get_requester(&self, requester_address: &ManagedAddress) -> SCResult<Requester> {
